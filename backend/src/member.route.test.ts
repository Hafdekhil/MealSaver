...
describe("Membres du foyer et totaux", () => {
  beforeAll(() => {
  ...
  it("retourne 401 sans session pour expense-totals", async () => {
    const response = await request(app)
      .get(`/api/households/${householdId}/expense-totals`);
    expect(response.status).toBe(401);
  });

  it("retourne 400 si le foyer est invalide pour expense-totals", async () => {
    const agent = await loginAs(ownerEmail);
    const response = await agent.get("/api/households/invalide/expense-totals");
    expect(response.status).toBe(400);
    expect(response.body.error).toBe("Foyer invalide");
  });

  it("retourne les totaux de dépenses par membre pour un membre du foyer", async () => {
    await prisma.shoppingItem.createMany({
      data: [
        { householdId, name: "Pain", normalizedName: "pain", price: 10, addedBy: "Household Owner" },
        { householdId, name: "Lait", normalizedName: "lait", price: 5, addedBy: "Household Member" },
      ],
    });

    const agent = await loginAs(ownerEmail);
    const response = await agent.get(`/api/households/${householdId}/expense-totals`);
