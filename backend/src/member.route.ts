...
memberRouter.get("/:householdId/expense-totals", async (req, res, next) => {
  try {
    const userId = Number(res.locals["userId"]);
    ...
    const dbExpenses = shoppingItems.map((item) => ({
      amount: item.price ?? 0,
      member: item.addedBy || "Général",
    }));

    const totals = calculateExpenseTotals(dbExpenses);
